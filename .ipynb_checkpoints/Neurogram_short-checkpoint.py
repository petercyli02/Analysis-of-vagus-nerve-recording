...
class Recording:
    ...
    @classmethod
    def open_record(cls, path, start, dur=None, load_from_file=False, load_multiple_files=False, downsample=1, port='Port B', map_path=None, verbose=1):
        """
...
def load_data_multich(path, start=0, dur=None, port='Port B', load_from_file=False, load_multiple_files=False, downsample=1, verbose=1):
    """This method loads...
...
#         """
    ...
    #----------------------------------------------
    def plot_signal(self,signal, ch, map_array, num_rows,num_columns, channels, text_label, text_title, ylim=None, figsize=(10, 15), dtformat='%M:%S.%f', savefigpath='', show_plot=False):
    ...
    def plot_freq_content(self, signal2plot, ch, nperseg=512, max_freq=10000, ylim=None, dtformat='%M:%S.%f', figsize=(10, 15), savefigpath='', show=False):
        """
        ...
        plt.subplot(313)
        powerSpectrum, freqenciesFound, time, imageAxis = plt.specgram(signal2plot['ch_%s'%ch], NFFT=nperseg, Fs=self.fs, mode='psd', scale='dB')
